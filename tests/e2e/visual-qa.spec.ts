...
test('desktop full-flow screen recording + main screens', async ({ page, context }) => {
  ...
  // Login OTP (code never shown in UI — FREE_LAUNCH sandbox)
  const phone = `+9665${String(Date.now()).slice(-8)}`;
  const uniqueTitle = `تدقيق مرئي FreeLaunch ${Date.now()}`;
  ...
  // Create + upload image + video → direct publish (FREE_LAUNCH)
  ...
  await page.screenshot({ path: `${ARTIFACTS}/desktop-07-ad-published.png`, fullPage: true });

  const adId = page.url().split('/ads/')[1]?.split('/')[0];
  expect(adId).toBeTruthy();

  // Admin recommendations + catalog pages (no pre-publish review in FREE_LAUNCH)
  ...
  await admin.screenshot({ path: `${ARTIFACTS}/desktop-08-admin-recommendations.png`, fullPage: true });
  await admin.goto(`${ADMIN}/pricing`);
  await admin.screenshot({ path: `${ARTIFACTS}/desktop-09-admin-pricing.png`, fullPage: true });
...
test('iphone viewport screens', async ({ browser }) => {
  ...
  await page.screenshot({ path: `${ARTIFACTS}/iphone-06-admin-recommendations.png`, fullPage: true });
  await context.close();
});
...
test('android viewport screens', async ({ browser }) => {
  ...
  await page.goto(`${ADMIN}/recommendations`);
  await page.screenshot({ path: `${ARTIFACTS}/android-06-admin-recommendations.png`, fullPage: true });
  await context.close();
});
