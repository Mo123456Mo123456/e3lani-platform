...
export default function AdminPage() {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (
        !["super_admin", "system_admin", "simulation_admin"].includes(
          next.user.role,
        )
      ) {
    ...
    <main className="dashboard">
      ...
      <section className="content">
        <header>
          <div>
            <h1>حالة النظام</h1>
            <p>قراءات مباشرة من قاعدة البيانات ومحرك المحاكاة</p>
          </div>
          <i>متصل</i>
        </header>
        <div className="metrics">
          ...
          <article>
            <span>المساهمات</span>
            <b>{stats?.contributions ?? "—"}</b>
          </article>
          <article>
            <span>الأحداث المحفوظة</span>
            <b>{stats?.events ?? "—"}</b>
          </article>
          <article>
            <span>Ticks المنفذة</span>
            <b>{stats?.ticks ?? "—"}</b>
          </article>
          <article>
            <span>تكلفة AI</span>
        ...
        <div className="panels">
          <article>
            ...
            <div className="actions">
              <button disabled={working} onClick={() => void tick(1)}>
                تشغيل سنة
              </button>
              <button disabled={working} onClick={() => void tick(10)}>
                تشغيل عشر سنوات
              </button>
              <button disabled title="لم يُفعّل rollback بعد">
                Rollback (غير مفعّل)
              </button>
            </div>
          </article>
          <article>
            ...
            <ul>
              <li>
                <i /> قاعدة البيانات متصلة
              </li>
              <li>
                <i /> سجل الأحداث يعمل
              </li>
              <li>
                <i /> المخرجات الحتمية مفعّلة
              </li>
              <li>
                <i /> بث Delta فوري
              </li>
