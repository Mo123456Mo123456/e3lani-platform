...
function makeEvent(
  ...
  return {
    ...input,
    id: deterministicId(
      `${state.planetId}:event:${state.tick + 1}:${sequence}:${input.type}:${input.regionId ?? "world"}`,
    ),
    planetId: state.planetId,
    tick: state.tick + 1,
    year: state.year + state.tickYears,
    createdAt: eventDate(state.tick + 1),
  };
}

function regionSuitability(region: PlanetRegion, species: SpeciesState): number {
  const temperatureFit =
    region.temperature >= 0.5
      ? 1 - Math.max(0, region.temperature - species.heatResistance)
      : 1 - Math.max(0, 0.5 - region.temperature - species.coldResistance * 0.5);
...
function evolveRegion(
  ...
  const volcanicPulse =
    region.biome === "volcanic" && random.chance(0.012) ? random.between(0.02, 0.08) : 0;
  const moisture = clamp(region.moisture + (neighborMoisture - region.moisture) * 0.015);
  ...
  const temperature = clamp(
    region.temperature + pollution * 0.0009 + volcanicPulse * 0.04 - region.vegetation * 0.0002,
  );
  const rainfall = clamp(region.rainfall + (moisture - region.rainfall) * 0.02);
  const carryingVegetation = clamp(
    region.fertility * moisture * (1 - pollution * 0.7) * (temperature > 0.12 ? 1 : 0.25),
  );
  ...
  return {
    ...
    surfaceWater: round(clamp(region.surfaceWater + rainfall * 0.003 - temperature * 0.0015)),
  };
}
...
function evolveCivilization(
  ...
  const foodProduction =
    region.fertility * region.surfaceWater * (0.65 + civilization.technology * 0.7);
  ...
  const population = Math.max(100, Math.round(civilization.population * (1 + growthRate)));
  const innovationGain =
    civilization.innovation * civilization.stability * 0.0015 * random.between(0.92, 1.08);
  const economy = clamp(
  ...
  return {
    ...civilization,
    population,
    food: round(clamp(civilization.food + (foodProduction - civilization.food) * 0.025)),
    economy: round(economy),
    technology: round(clamp(civilization.technology + innovationGain)),
    military: round(
      clamp(civilization.military + civilization.aggression * scarcity * 0.001 - 0.00025),
    ),
    stability: round(
      clamp(civilization.stability + (civilization.food - 0.4) * 0.002 - scarcity * 0.001),
    ),
    pollution: round(clamp(civilization.pollution + civilization.technology * 0.0004)),
  };
}
...
function getNeighbors(regions: PlanetRegion[], index: number): PlanetRegion[] {
  ...
  return positions
    .map(([neighborX, neighborY]) => regions[(neighborY ?? 0) * width + (neighborX ?? 0)])
...
export function simulateTick(current: WorldState): {
  ...
  const species = current.species
    .map((speciesState) => {
      ...
      const capacity = Math.max(10, speciesState.carryingCapacity * suitability);
      const growth =
  ...
  const changedRegions = regions
    .filter((region, index) => JSON.stringify(region) !== JSON.stringify(current.regions[index]))
    .map((region) => ({ ...region }));
  const changedCivilizations = civilizations
    .filter(
      (civilization, index) =>
        JSON.stringify(civilization) !== JSON.stringify(current.civilizations[index]),
    )
  ...
  const highPollutionRegion = regions.find(
    (region, index) =>
      region.pollution >= 0.25 && (current.regions[index]?.pollution ?? 0) < 0.25,
  );
    ...
    events.push(
      makeEvent(current, events.length, {
        ...
        actorIds: civilizations
          .filter((civilization) => civilization.regionId === highPollutionRegion.id)
...
export function applyContribution(
  ...
  const regionIndex = current.regions.findIndex((region) => region.id === regionId);
  ...
  const spread = makeEvent(current, 1, {
    type: analysis.category === "plant" ? "VEGETATION_SPREAD" : "SPECIES_CREATED",
    regionId,
    cause: `${analysis.name} passed validation and habitat suitability was ${Math.round(
      biomeFit * 100,
    )}%`,
    actorIds: [contributionId],
    payload: { contributionId, biomeFit, possibleBiomes: analysis.possibleBiomes },
    confidence: round(0.55 + biomeFit * 0.4),
    directImpact: { establishmentProbability: round(biomeFit * (0.6 + growthRate * 0.35)) },
    rootContributionId: contributionId,
    causedByEventIds: [added.id],
  });
  const events = [added, spread];
  return {
    state: next,
    events,
    delta: {
...
export function replayEvents(initial: WorldState, events: WorldEvent[]): WorldState {
  ...
  for (const event of events) {
    if (event.type === "CONTRIBUTION_ADDED") {
      ...
      if (region) {
        state.regions = state.regions.map((current) => (current.id === region.id ? region : current));
        state.version += 1;
      }
    }
    if (event.type === "TICK_COMPLETED") {
      const patch = event.payload.statePatch as
        ...
        | undefined;
      if (!patch) continue;
      const regions = new Map(patch.changedRegions.map((region) => [region.id, region]));
      const civilizations = new Map(
        patch.changedCivilizations.map((civilization) => [civilization.id, civilization]),
      );
      state = {
        ...state,
        ...
        tick: patch.tick,
        year: patch.year,
        regions: state.regions.map((region) => regions.get(region.id) ?? region),
