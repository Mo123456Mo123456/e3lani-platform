...
const log = new Logger('EnsureSeed');
...
/**
 * Idempotent staging/bootstrap seed. Runs when categories are missing so a
 * freshly provisioned Render Postgres can serve create-ad flows without a
 * manual `prisma db seed`.
 */
export async function ensureSeedData(prisma: PrismaClient): Promise<void> {
  ...
  log.log(`Categories empty/incomplete (${categoryCount}); applying bootstrap seed`);
