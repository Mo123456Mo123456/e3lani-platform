...
export type AccountType = "viewer" | "advertiser" | "brand";
...
export type AdStatus =
  ...
  | "expired"
  | "removed";
export type ContactType = "store" | "product" | "whatsapp" | "phone";
...
export type City = { id: string; ar: string; en: string; region: string };
export type AdMedia = {
  id: string;
  kind: "image" | "video";
  uri: string;
...
export function transitionToPendingReview(ad: Ad, items: PromotionCode[]) {
  ...
  return {
    ...
    promotions: [...new Set(items)],
  };
}
export function moderatePendingAd(
  ad: Ad,
...
export function extendAdPeriod(ad: Ad) {
  if ((ad.status !== "active" && ad.status !== "paused") || !ad.expiresAt)
    return ad;
...
export function republishExpiredAd(ad: Ad, nowIso: string) {
  ...
  if (
    ...
    !canRepublish(ad.lastRepublishedAt, nowMs)
  )
    return ad;
  return {
    ...ad,
...
export const seedAds: Ad[] = [
  {
    ...
    contacts: [
      { type: "store", value: "https://example.com" },
      { type: "whatsapp", value: "+966500000000" },
    ],
    status: "active",
    revision: 1,
    verified: true,
