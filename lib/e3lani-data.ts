...
export type Ad={id:string;ownerId:string;brandId?:string;title:string;description:string;categoryId:string;cityId:string;countryCode?:string;media:AdMedia[];contacts:AdContact[];status:AdStatus;revision:number;verified:boolean;featured:boolean;sponsored:boolean;createdAt:string;activatedAt?:string;expiresAt?:string;lastRepublishedAt?:string;promotions:PromotionCode[];aiLabel?:AiModerationLabel;paymentStatus?:"not_required"|"pending"|"paid"|"failed";priceSnapshot?:{basePrice:number;discount:number;tax:number;finalPrice:number;currency:string;freeReason:string|null}};
